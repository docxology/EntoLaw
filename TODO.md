...
```
uv run pytest tests/ -m "not live" --cov=src --cov-fail-under=90  # 208 passed; 1 skipped; Required test coverage of 90% reached. Total coverage: 97.36%
uv run ruff check src scripts tests                               # All checks passed!
uv run python scripts/check_module_map.py                         # module map: 0 violations
uv run python scripts/check_docs_inventory.py                     # inventory passed: config 12, docs 6, scripts 14, skills 0, source_modules 26, tests 22; markdown files checked: 56
